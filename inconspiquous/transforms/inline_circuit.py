    ...
    def apply(self, ctx: MLContext, module):
        ...
        for circuit_op in list(module.walk(QSSACircuitOp)):
            ...
            for use in uses:
                ...
                for circuit_arg, gate_arg in zip(
                    circuit_block.args, dyn_gate_op.qubits
                ):
                ...
                assert isinstance(return_op, QSSAReturnOp), (
                    "Circuit must end with qssa.return"
                )

                # 3. Replace all uses of the dynamic gate's results with the circuit's return values
                for gate_result, return_val in zip(
                    dyn_gate_op.results, return_op.qubits
                ):
                    gate_result.replace_all_uses_with(return_val)

                # 4. Move the operations from the circuit body to before the gate call
                for op_in_circuit in list(circuit_block.ops)[
                    :-1
                ]:  # All ops except the qssa.return
